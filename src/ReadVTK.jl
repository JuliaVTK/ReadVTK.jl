...
function VTKFile(filename)
  ...
  VTKFile(filename, xml_file, file_type, version, byte_order, compressor, appended_data,
          n_points, n_cells)
end
...
function piece(vtk_file::VTKFile)
  LightXML.root(vtk_file.xml_file)[vtk_file.file_type][1]["Piece"][1]
end
...
function get_coordinate_data(pvtk_file::PVTKFile)
  get_data_section.(pvtk_file.vtk_files, "Coordinates")
end
...
function Base.show(io::IO, data_array::VTKDataArray)
  print(io, "VTKDataArray(\"", data_array.name, "\")")
end
...
function get_cells(vtk_file::VTKFile)
  ...
  VTKCells(get_data(connectivity) + oneunit.(get_data(connectivity)),
           get_data(offsets),
           get_data(types))
end
